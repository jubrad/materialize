...
class InsertLoad(threading.Thread):
    ...
    With `burst_period == 0` the load is continuous. Otherwise all threads
    write only during the first half of each wall-clock-aligned period and
    idle the second half. The synchronized quiet phase lets the pool build an
    idle buffer (and lets a node drain kill connections while nothing is
    watching), so each burst onset measures how expensive it is to reassemble
    connections under sudden demand. That alternation is the production shape
    of the incident this workflow reproduces."""
    ...
    def run(self) -> None:
        ...
        while not self.stop_event.is_set():
            if (
                self.burst_period > 0
                and (time.time() % self.burst_period) >= self.burst_period / 2
            ):
...
def workflow_pool_exhaustion(c: Composition, parser: WorkflowArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="alternate all insert threads between a half-period burst and a "
        "half-period quiet phase (0 = continuous load)",
    )
    parser.add_argument(
        "--crdb-latency-ms",
        type=int,
        default=0,
        help="per-direction latency injected between environmentd and the "
        "CRDB load balancer via toxiproxy, to give connection establishment "
        "(several round trips) a production-like cost relative to queries "
        "(one round trip). 0 connects directly to the balancer.",
    )
    args = parser.parse_args()
    ...
    crdb_addr = "crdb-lb:26257"
    if args.crdb_latency_ms > 0:
        c.up("crdb-lb", "toxiproxy")
        toxiproxy_port = c.port("toxiproxy", 8474)

        def toxiproxy_post(path: str, body: dict) -> None:
            req = urllib.request.Request(
                f"http://localhost:{toxiproxy_port}{path}",
        ...
        toxiproxy_post(
            "/proxies",
            {"name": "crdb", "listen": "0.0.0.0:26257", "upstream": "crdb-lb:26257"},
        )
        for stream in ["upstream", "downstream"]:
            toxiproxy_post(
                "/proxies/crdb/toxics",
                {
                    ...
                    "type": "latency",
                    "stream": stream,
                    "attributes": {"latency": args.crdb_latency_ms, "jitter": 0},
                },
            )
        crdb_addr = "toxiproxy:26257"
    ...
    with c.override(
        Materialized(
            ...
            options=[
                f"--persist-consensus-url=postgres://root@{crdb_addr}?options=--search_path=consensus",
                f"--timestamp-oracle-url=postgres://root@{crdb_addr}?options=--search_path=tsoracle",
            ],
        ...
        loaders = [
            InsertLoad(
                ...
                i,
                args.insert_threads,
                burst_period=args.burst_period_secs,
            )
