...
def test_create_privatelink_connection(mz: MaterializeApplication) -> None:
    ...
    mz.environmentd.sql_query("SELECT id FROM mz_connections WHERE name = 'kafkaconn_alt'")[
        0
    ][0]
    ...
    with pytest.raises(
        ProgrammingError, match= "invalid CONNECTION: PORT in AWS PRIVATELINK is only supported for kafka",
    ):
        mz.environmentd.sql(
            dedent(
                """\
