...
pub static MZ_UNSTABLE_CLUSTER_REPLICAS: Lazy<BuiltinTable> = Lazy::new(|| BuiltinTable {
    name: "mz_unstable_cluster_replicas",
    schema: MZ_INTERNAL_SCHEMA,
    oid: oid::TABLE_MZ_UNSTABLE_CLUSTER_REPLICAS_OID,
    desc: RelationDesc::empty().with_column("id", ScalarType::String.nullable(false)),
    is_retained_metrics_object: false,
    access: vec![PUBLIC_SELECT],
});
...
pub static BUILTINS_STATIC: Lazy<Vec<Builtin<NameReference>>> = Lazy::new(|| {
    ...
    builtins.append(&mut vec![
        ...
        Builtin::Table(&MZ_UNSTABLE_CLUSTER_REPLICAS),
