...
if (typeof (globalThis as Record<string, unknown>)["WebTransport"] === "undefined") {
  const { WebTransport, quicheLoaded } = await import("@fails-components/webtransport");
  await quicheLoaded;
  (globalThis as Record<string, unknown>)["WebTransport"] = WebTransport;
}
...
function certFileToHash(pemPath: string): Array<{ algorithm: string; value: Uint8Array<ArrayBuffer> }> {
  const pem = readFileSync(pemPath, "utf8");
  const match = /-----BEGIN CERTIFICATE-----\r?\n([\s\S]+?)\r?\n-----END CERTIFICATE-----/.exec(pem);
  if (!match) throw new Error(`No certificate found in ${pemPath}`);
...
const rawTypes: pg.CustomTypesConfig = {
  getTypeParser: () => (val: string) => val,
};

let pgClient: pg.Client | undefined;
let wtClient: WtClient | undefined;
...
interface NormalizedResult {
  fields: Array<{ name: string; dataTypeOid: number }>;
  rows: (string | null)[][];
  commandTag: string;
}
...
function normalizeWt(r: import("./client.js").QueryResult): NormalizedResult {
  return {
...
function normalizePg(r: pg.QueryResult): NormalizedResult {
  ...
  return {
    ...
    rows: (r.rows as (string | null)[][]).map((row) =>
...
function clients(): { pg: pg.Client; wt: WtClient } {
  if (!pgClient || !wtClient) throw new Error("clients not initialized — before() failed");
  return { pg: pgClient, wt: wtClient };
}

async function runBoth(sql: string): Promise<{ pg: NormalizedResult; wt: NormalizedResult }> {
...
async function runBothExtended(
  sql: string,
  params: (string | null)[],
): Promise<{ pg: NormalizedResult; wt: NormalizedResult }> {
  const { pg: p, wt: w } = clients();
  const [pgResult, wtResult] = await Promise.all([
...
async function runBothExpectError(sql: string): Promise<{
  pg: NormalizedError;
  wt: NormalizedError;
}> {
  const { pg: p, wt: w } = clients();
  const [pgErr, wtErr] = await Promise.all([
    p.query(sql).then(() => null, (e: Error & { code?: string }) => e),
    w.query(sql).then(() => null, (e: unknown) => e),
  ]);
  if (!pgErr) throw new Error("pg did not error");
  if (!wtErr) throw new Error("wt did not error");
  ...
  for (const { name, sql } of cases) {
    test(name, async () => {
      ...
      assert.deepEqual(
        wtR.fields,
        pgR.fields,
        `field metadata mismatch for: ${sql}`,
      );
      assert.deepEqual(
        wtR.rows,
        pgR.rows,
        `row data mismatch for: ${sql}`,
      );
    });
  }
});
...
describe("field metadata — OIDs", () => {
  ...
  for (const { name, sql } of cases) {
    test(name, async () => {
      const { pg: pgR, wt: wtR } = await runBoth(sql);
      assert.deepEqual(
        wtR.fields.map((f) => f.dataTypeOid),
        pgR.fields.map((f) => f.dataTypeOid),
        `OID mismatch for: ${sql}`,
      );
    });
  }
});
...
describe("extended query protocol — parameterized queries", () => {
  const cases: Array<{ name: string; sql: string; params: (string | null)[] }> = [
    {
  ...
  for (const { name, sql } of cases) {
    test(name, async () => {
      ...
      assert.equal(
        wtE.code,
        pgE.code,
        `SQLSTATE mismatch for: ${sql} (pg=${pgE.code}, wt=${wtE.code})`,
      );
    });
  }
});
...
describe("NULL edge cases", () => {
  test("null is distinct from empty string", async () => {
    const { pg: pgR, wt: wtR } = await runBoth(
      "SELECT NULL AS a, '' AS b, 'null' AS c",
    );
  ...
  test("all-null row", async () => {
    const { pg: pgR, wt: wtR } = await runBoth(
      "SELECT NULL::int4 AS a, NULL::text AS b, NULL::bool AS c",
    );
    assert.deepEqual(wtR.rows, pgR.rows);
  ...
  test("null mixed with values across multiple rows", async () => {
    const { pg: pgR, wt: wtR } = await runBoth(
      "SELECT * FROM (VALUES (1, NULL), (NULL, 'x'), (3, 'y')) AS t(a, b)",
    );
    assert.deepEqual(wtR.rows, pgR.rows);
  });
});
...
describe("query cancellation", () => {
  ...
  test("CancelRequest interrupts a blocking query with SQLSTATE 57014", async () => {
    ...
    const queryPromise = wt
      ...
      .then(
        () => { throw new Error("expected query to be cancelled, not succeed"); },
        (e: unknown) => { caught = e; },
      );
...
describe("LISTEN / NOTIFY", () => {
  ...
  test("listen() registers and can be aborted cleanly", async () => {
    ...
    try {
      ...
      if (e instanceof PgError && e.code === "42601") return;
      throw e;
    }
