...
function generateNonce(byteLength = 24): string {
 ...
 */
export function scram256ClientFirst(username: string): {
 ...
 */
function parseServerFirst(data: Uint8Array): {
  serverNonce: string;
  salt: Uint8Array;
  iterations: number;
  ...
  if (!serverNonce || !saltB64 || !iterStr) {
    throw new ScramError(
      `Invalid server-first-message: missing r/s/i in "${serverFirstMessage}"`,
    );
  }
 ...
 */
export async function scram256ClientFinal(
 ...
 */
export function scram256VerifyServerFinal(
  data: Uint8Array,
...
export function cleartextPasswordMessage(password: string): import("./codec.js").PasswordMessage {
