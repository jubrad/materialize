...
/// `SessionVars::check_read_only` rejects a direct `SET`, and the
/// `PlannedAlterRoleOption::Variable` arm of `generate_rbac_requirements` requires superuser.
/// Drop any one and a role can lift its own ceiling.
