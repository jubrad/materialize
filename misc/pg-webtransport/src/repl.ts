...
import { Client, PgError, type Field, type QueryResult } from "./client.js";

// ---------------------------------------------------------------------------
// WebTransport availability — use built-in (Node >=22) or polyfill.
// ---------------------------------------------------------------------------

async function ensureWebTransport(): Promise<void> {
  ...
  try {
    ...
    process.stderr.write(
      "Error: WebTransport is not available.\n" +
...
function parseArgs(argv: string[]): Args {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    switch (arg) {
      ...
      case "--user": case "-U": args.user = next; i++; break;
      case "--password": case "-P": args.password = next; i++; break;
      case "--token":          args.token = next; i++; break;
      case "--database": case "-d": args.database = next; i++; break;
      case "--command": case "-c":  args.command = next; i++; break;
      case "--server-cert":    args.serverCert = next; i++; break;
      case "--no-tls-verify":  args.noTlsVerify = true; break;
 ...
 */
async function certFileToHash(pemPath: string): Promise<Uint8Array<ArrayBuffer>> {
...
function renderTable(result: QueryResult): string {
  ...
  if (fields.length === 0) {
    if (commandTag) lines.push(commandTag);
    return lines.join("\n");
  }
...
function makeSubscribeRenderer(fields: Field[]): (row: (string | null)[]) => string {
  const headers = fields.map((f) => f.name);
...
async function runRepl(client: Client): Promise<void> {
  ...
  while (true) {
    ...
    try {
      ...
      if (isSubscribe) {
        ...
        let renderer: ((row: (string | null)[]) => string) | null = null;
        ...
        try {
          await client.subscribe(
            sql,
            (fields, row) => {
              if (!renderer) renderer = makeSubscribeRenderer(fields);
        ...
        if (timing) process.stdout.write(`Time: ${ms.toFixed(3)} ms\n`);
        continue;
      }
      ...
      if (err instanceof PgError) {
        ...
        if (err.code) process.stderr.write(`DETAIL: SQLSTATE ${err.code}\n`);
      } else {
        process.stderr.write(
...
async function main(): Promise<void> {
  ...
  let serverCertificateHashes: Array<{ algorithm: string; value: Uint8Array<ArrayBuffer> }> | undefined;
